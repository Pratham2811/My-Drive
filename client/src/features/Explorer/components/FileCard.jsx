...
import {
  ...
  formatFileSize,
  formatFileDate,
} from "@/features/file/constants/file.constants";
import { useDispatch, useSelector } from "react-redux";
import { openModal } from "../explorerSlice";
import { cn } from "@/lib/utils";
import { getGoogleDriveFile } from "../explorerThunk";

/**
 * File Card Component
 * Renders an individual file in List or Grid view using Redux dispatch.
 */
export const FileCard = ({ file, viewMode }) => {
  ...
  const handleView = (e) => {
    e?.preventDefault();
    dispatch(
      openModal({ modalName: "fileViewer", item: file, itemType: "file" }),
    );
  };
  ...
  const handleRename = (e) => {
    e?.stopPropagation();
    dispatch(openModal({ modalName: "rename", item: file, itemType: "file" }));
  };

  const handleDelete = (e) => {
    e?.stopPropagation();
    dispatch(openModal({ modalName: "delete", item: file, itemType: "file" }));
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-full space-y-1">
        <h3
          className="text-sm font-medium text-slate-700 truncate group-hover:text-indigo-600 transition-colors"
          title={file.name}
        >
          {file.name}
        </h3>
