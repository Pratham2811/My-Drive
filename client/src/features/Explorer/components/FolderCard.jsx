...
export const FolderCard = ({ folder, viewMode }) => {
  ...
  const handleRename = (e) => {
    e.stopPropagation();
    dispatch(openModal({ modalName: "rename", item: folder, itemType: "folder" }));
  };

  const handleDelete = (e) => {
    e.stopPropagation();
    dispatch(openModal({ modalName: "delete", item: folder, itemType: "folder" }));
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-full mt-auto">
        <h3
          className="text-sm font-semibold text-slate-700 truncate group-hover:text-indigo-600 transition-colors"
          title={folder.name}
        >
          {folder.name}
        </h3>
